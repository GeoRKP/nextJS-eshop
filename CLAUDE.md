...
```bash
npm run dev          # Dev server (Turbopack is default in Next.js 16)
...
- **Next.js 16** (App Router) with **React 19.2** and **TypeScript** (strict mode)
- **Prisma 6.19** ORM with **Neon serverless PostgreSQL** (WebSocket adapter)
...
- **Zod v4** for validation, **react-hook-form** for forms (standardSchemaResolver)
...
Next.js 16 renamed middleware to proxy. Composes NextAuth authorization. Sets `sessionCartId` cookie (UUID) for guest cart tracking. Protects routes via regex patterns. Runs in Node.js runtime (not Edge).
